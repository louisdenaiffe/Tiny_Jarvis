...
def main():
    ...
    while True:
        ...
        if not audio_file:  # if audio recordings are found in recordings/
            time.sleep(0.2)
            continue
        process_recording(audio_file, d, recipes)
...
def process_recording(audio_file, d, recipes):
    try:
        ...
        if not prompt.strip():  # if the user didn't say anything, if faster-whisper returned nothing
            ...
            return # skips the rest of the code in this iteration and goes back to the top of the while loop
        if prompt.startswith("Cooking"):
            recipe = get_random_recipe(recipes)
            d.show_text(recipe['Ingredients'])
            raw_token_stream = [format_cooking_sentence(recipe)]  # we put it in a list so sentence_buffer can iterate over it without breaking
        else:
